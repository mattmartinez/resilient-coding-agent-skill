...
setup() {
  ...
  # Create a minimal manifest (key=value format)
...
CLAUDE_PID=\$!
echo "\$CLAUDE_PID" > "\$TASK_TMPDIR/pid"
manifest_set "\$TASK_TMPDIR/manifest" pid "\$CLAUDE_PID" status running
wait \$CLAUDE_PID
ECODE=\$?
...
SCRIPT
  ...
  status=$(grep '^status=' "$TASK_TMPDIR/manifest" | cut -d= -f2-)
  [ "$status" = "completed" ]
  [ -f "$TASK_TMPDIR/done" ]
}
